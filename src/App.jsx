          ...
          <ul className="flex items-center gap-2 list-none font-nav text-[16px] font-bold text-[#212529]">
            ...
            <li className="relative group" onMouseEnter={() => { setActiveMenu('services'); setActiveTab(0); }} onMouseLeave={() => setActiveMenu(null)}>
              <Link to="/services" className={cn("px-4 py-4 hover:text-[#0c67c4] flex items-center gap-1 transition-colors text-black", (activeMenu === 'services' || location.pathname === '/services') && "text-[#0c67c4]")}>
                Services <ChevronDown size={14} className={cn("transition-transform", activeMenu === 'services' && "rotate-180")} />
              </Link>
              <AnimatePresence>
                {activeMenu === 'services' && (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, y: 15 }} 
                    className="absolute top-[85%] left-1/2 -translate-x-1/2 w-[1050px] bg-white shadow-[0_10px_60px_rgba(0,0,0,0.1)] rounded-2xl flex p-6 border border-gray-100 z-[1100]"
                  >
                    <div className="w-[340px] bg-[#f3f9ff] rounded-xl p-2 flex flex-col gap-1 h-[450px]">
                      {servicesData.map((s, i) => (
                        <button key={i} onMouseEnter={() => setActiveTab(i)} className={cn("w-full text-left p-4 rounded-xl transition-all font-bold text-[14px] flex items-center justify-between", activeTab === i ? "bg-white text-[#0c67c4] shadow-sm" : "text-[#495057] hover:text-[#0c67c4]")}>
                          {s.title} {activeTab === i && <ChevronRight size={14} />}
                        </button>
                      ))}
                    </div>
                    <div className="flex-grow pl-10 py-6 grid grid-cols-2 gap-10 h-[450px] overflow-y-auto">
            ...
            <li className="relative group" onMouseEnter={() => setActiveMenu('partnerships')} onMouseLeave={() => setActiveMenu(null)}>
              ...
              <AnimatePresence>
                {activeMenu === 'partnerships' && (
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 15 }} className="absolute top-[100%] left-[-200px] w-[850px] bg-white shadow-[0_10px_60px_rgba(0,0,0,0.1)] rounded-2xl flex p-12 border border-gray-100 items-center">
            ...
            <li className="relative group" onMouseEnter={() => { setActiveMenu('industries'); setActiveTab(0); }} onMouseLeave={() => setActiveMenu(null)}>
              ...
              <AnimatePresence>
                {activeMenu === 'industries' && (
                  <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 15 }} className="absolute top-[100%] left-[-350px] w-[950px] bg-white shadow-[0_10px_60px_rgba(0,0,0,0.1)] rounded-2xl flex p-8 border border-gray-100">
