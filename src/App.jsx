              ...
              <AnimatePresence>
                {activeMenu === 'services' && (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, y: 15 }} 
                    className="absolute top-[90%] left-0 md:left-[-20px] w-[1050px] bg-white shadow-[0_10px_60px_rgba(0,0,0,0.1)] rounded-2xl flex p-6 border border-gray-100 z-[1100]"
                  >
                    <div className="w-[380px] bg-[#f3f9ff] rounded-xl p-3 flex flex-col gap-1.5 h-[480px]">
                      {servicesData.map((s, i) => (
                        <button key={i} onMouseEnter={() => setActiveTab(i)} className={cn("w-full text-left px-4 py-3 rounded-xl transition-all font-bold text-[13px] flex items-center justify-between min-h-[56px]", activeTab === i ? "bg-white text-[#0c67c4] shadow-sm" : "text-[#495057] hover:text-[#0c67c4]")}>
                          <span className="leading-tight pr-2">{s.title}</span> {activeTab === i && <ChevronRight size={14} className="shrink-0" />}
                        </button>
                      ))}
                    </div>
                    <div className="flex-grow pl-10 py-6 grid grid-cols-2 gap-x-10 gap-y-8 h-[480px] overflow-y-auto">
...
const TeamProfilePage = () => {
  ...
  const team = [
    ...
    { id: "zack-ristov", name: "Zack Ristov", role: "Co-Founder & Director", img: "https://aussiemigrationservices.com.au/wp-content/uploads/2024/01/AI_Zack_Ristov_891_edited-800x800-1.webp", linkedin: "https://www.linkedin.com/in/zack-ristov/", email: "contact@datamatrixiq.com", phone: "+61 449 602 007", bio: "Zack is a Co-Founder and Director at DataMatrixIQ, bringing over 20 years of experience in business development, IT consultancy, and project management. He holds an MBA from RMIT University and is instrumental in driving the company's growth and client satisfaction strategies across Australia and New Zealand." },
